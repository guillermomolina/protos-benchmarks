...
MODE=${1:-}
OUT=${2:-}
CONFIG="$ROOT/config/perf003a-a4f.json"
case "$MODE" in --smoke|--run) ;; *) echo "usage: scripts/perf003a_a4f_prepared_replay_activation_experiment.sh <--smoke|--run> <output-dir>" >&2; exit 2;; esac
...
CONTROL_IMAGE="protos-benchmarks-perf003a-a4f-control:${PROTOS_REVISION:0:12}"
BOUNDARY_IMAGE="protos-benchmarks-perf003a-a4f-boundary:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4f"
echo "PERF003-A4f controlled invokePrepared plus replay-activation boundary experiment"
echo "mode=$MODE"
...
echo "phase=01 build experimental invokePrepared plus replay-activation boundary image"
...
docker run --rm --network none --entrypoint sh "$BOUNDARY_IMAGE" -c \
  "grep -B1 -F 'private static Object invokePrepared(' /opt/protos/src/main/java/com/guillermomolina/protos/execution/ProtosClosureInvoker.java | grep -q -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary' && \
   grep -B1 -F 'public ProtosActivation invocationActivation(Supplier<ProtosActivation> factory)' /opt/protos/src/main/java/com/guillermomolina/protos/runtime/ProtosEvaluatorContinuation.java | grep -q -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary'"
...
echo "phase=04 invokePrepared plus replay-activation boundary TraceCompilation"
...
if cg==0:
 ...
 c,r='SUPPORTED','replay-activation boundary eliminated the A4a residual GraphTooBig'
elif bg<ref_graph:
 c,r='SUPPORTED','replay-activation boundary reduced GraphTooBig occurrences relative to A4a'
elif bs and max(x[1] for x in bs)<ref_shape[1]:
 c,r='SUPPORTED','replay-activation boundary reduced failing graph size relative to A4a'
elif bg==ref_graph and bs==[ref_shape]:
 c,r='NOT_SUPPORTED','replay-activation boundary left the A4a residual graph unchanged'
else:
 c,r='INCONCLUSIVE','replay-activation boundary changed the A4a residual diagnostics without a directional result'

open(out,'w',encoding='utf-8').write(f'conclusion={c}\nreason={r}\n')
print(f'A4F_HYPOTHESIS={c}')
print(f'A4F_REASON={r}')
PYCON
docker image inspect "$CONTROL_IMAGE" >"$OUT/control-image.json"
docker image inspect "$BOUNDARY_IMAGE" >"$OUT/boundary-image.json"
docker --version >"$OUT/docker-version.txt"
echo "PERF003A_A4F_EXPERIMENT: PASS"
