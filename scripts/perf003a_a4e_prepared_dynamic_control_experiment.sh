...
MODE=${1:-}
OUT=${2:-}
CONFIG="$ROOT/config/perf003a-a4e.json"
case "$MODE" in --smoke|--run) ;; *) echo "usage: scripts/perf003a_a4e_prepared_dynamic_control_experiment.sh <--smoke|--run> <output-dir>" >&2; exit 2;; esac
...
CONTROL_IMAGE="protos-benchmarks-perf003a-a4e-control:${PROTOS_REVISION:0:12}"
BOUNDARY_IMAGE="protos-benchmarks-perf003a-a4e-boundary:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4e"
echo "PERF003-A4e controlled invokePrepared plus dynamic-control boundary experiment"
echo "mode=$MODE"
...
echo "phase=01 build experimental invokePrepared plus dynamic-control boundary image"
...
docker run --rm --network none --entrypoint sh "$BOUNDARY_IMAGE" -c \
  "grep -B1 -F 'private static Object invokePrepared(' /opt/protos/src/main/java/com/guillermomolina/protos/execution/ProtosClosureInvoker.java | grep -q -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary' && \
   grep -B1 -F 'public void inheritDynamicControlState(' /opt/protos/src/main/java/com/guillermomolina/protos/runtime/ProtosActivation.java | grep -q -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary'"
...
echo "PERF003A_A4E_EXPERIMENT: PASS"
