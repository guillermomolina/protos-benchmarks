...
MODE=${1:-}
OUT=${2:-}
CONFIG="$ROOT/config/perf003a-a4d.json"
...
CONTROL_IMAGE="protos-benchmarks-perf003a-a4d-control:${PROTOS_REVISION:0:12}"
BOUNDARY_IMAGE="protos-benchmarks-perf003a-a4d-boundary:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4d"
echo "PERF003-A4d controlled immediate-activation boundary experiment"
echo "mode=$MODE"
...
echo "phase=01 build experimental immediate-activation boundary image"
...
docker run --rm --network none --entrypoint sh "$BOUNDARY_IMAGE" -c \
  "grep -A1 -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary' /opt/protos/src/main/java/com/guillermomolina/protos/runtime/ProtosActivation.java | grep -q -F 'public static ProtosActivation forImmediateMethodInvocation('"
...
echo "phase=04 immediate-activation boundary TraceCompilation"
...
elif bg==0: c,r='SUPPORTED','immediate-activation boundary eliminated GraphTooBig'
elif bg<cg: c,r='SUPPORTED','immediate-activation boundary reduced GraphTooBig occurrences'
elif cs and bs and max(x[1] for x in bs)<max(x[1] for x in cs): c,r='SUPPORTED','immediate-activation boundary reduced failing graph size'
elif cg==bg and cs==bs: c,r='NOT_SUPPORTED','immediate-activation boundary left GraphTooBig count and graph shape unchanged'
else: c,r='INCONCLUSIVE','immediate-activation boundary changed diagnostics without a directional result'
open(out,'w',encoding='utf-8').write(f'conclusion={c}\nreason={r}\n')
print(f'A4D_HYPOTHESIS={c}'); print(f'A4D_REASON={r}')
PYCON
docker image inspect "$CONTROL_IMAGE" >"$OUT/control-image.json"
docker image inspect "$BOUNDARY_IMAGE" >"$OUT/boundary-image.json"
docker --version >"$OUT/docker-version.txt"
echo "PERF003A_A4D_EXPERIMENT: PASS"
