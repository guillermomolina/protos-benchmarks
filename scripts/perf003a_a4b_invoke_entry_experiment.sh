...
MODE=${1:-}
OUT=${2:-}
CONFIG="$ROOT/config/perf003a-a4b.json"

case "$MODE" in
    --smoke|--run) ;;
    *)
        echo "usage: scripts/perf003a_a4b_invoke_entry_experiment.sh <--smoke|--run> <output-dir>" >&2
        exit 2
        ;;
esac
...
CONTROL_IMAGE="protos-benchmarks-perf003a-a4b-control:${PROTOS_REVISION:0:12}"
BOUNDARY_IMAGE="protos-benchmarks-perf003a-a4b-boundary:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4b"

echo "PERF003-A4b controlled closure invoke-entry boundary experiment"
echo "mode=$MODE"
...
build_variant() {
    ...
    if [ "$rc" -ne 0 ]; then
        ...
        echo "---- ${prefix} build stderr tail ----" >&2
        tail -80 "$OUT/${prefix}-build.stderr" >&2 || true
        echo "---- ${prefix} build stdout tail ----" >&2
        tail -40 "$OUT/${prefix}-build.stdout" >&2 || true
        echo "---- end ${prefix} build log tail ----" >&2
        return "$rc"
    fi
}
...
run_trace() {
    ...
    [ "$rc" -eq 0 ] || {
        ...
        tail -80 "$OUT/${prefix}-trace.stderr" >&2 || true
        exit 30
    }
}
...
summarize_trace() {
    local prefix=$1
    python3 - "$OUT/${prefix}-trace.stderr" "$prefix" <<'PY'
import re, sys
...
print(f"{prefix}_graph_shapes=" + (
    ",".join(f"{n}:{g}:{limit}" for n,g,limit in unique) if unique else "NONE"
))
PY
}

echo "phase=01 build experimental boundary image"
...
docker run --rm --network none --entrypoint sh "$BOUNDARY_IMAGE" -c \
    "grep -B4 -A1 -F 'ProtosActivation caller) {' \
     /opt/protos/src/main/java/com/guillermomolina/protos/execution/ProtosClosureInvoker.java \
     | grep -q -F '@com.oracle.truffle.api.CompilerDirectives.TruffleBoundary'"
...
[ "${control_graph:-0}" -gt 0 ] || {
    echo "EXPERIMENT_PRECONDITION: FAIL control did not reproduce GraphTooBig" >&2
    exit 40
}

echo "phase=04 boundary TraceCompilation"
run_trace "$BOUNDARY_IMAGE" boundary
boundary_summary=$(summarize_trace boundary)
printf '%s\n' "$boundary_summary"

python3 - "$OUT/control-trace.stderr" "$OUT/boundary-trace.stderr" "$OUT/conclusion.txt" <<'PY'
import re, sys
control_path,boundary_path,out_path=sys.argv[1:]
def stats(path):
    text=open(path, encoding="utf-8", errors="replace").read()
    graph=len(re.findall(r"GraphTooBig", text, re.I))
    shapes=sorted({
    ...
    return graph,shapes
cg,cs=stats(control_path)
bg,bs=stats(boundary_path)
if cg == 0:
    conclusion,reason="INCONCLUSIVE","control did not reproduce GraphTooBig"
elif bg == 0:
    conclusion,reason="SUPPORTED","invoke-entry boundary eliminated GraphTooBig"
elif bg < cg:
    conclusion,reason="SUPPORTED","invoke-entry boundary reduced GraphTooBig occurrences"
elif cs and bs and max(x[1] for x in bs) < max(x[1] for x in cs):
    conclusion,reason="SUPPORTED","invoke-entry boundary reduced failing graph size"
elif cg == bg and cs == bs:
    conclusion,reason="NOT_SUPPORTED","invoke-entry boundary left GraphTooBig count and graph shape unchanged"
else:
    conclusion,reason="INCONCLUSIVE","invoke-entry boundary changed diagnostics without a directional result"
with open(out_path,"w",encoding="utf-8") as fh:
    fh.write(f"conclusion={conclusion}\nreason={reason}\n")
print(f"A4B_HYPOTHESIS={conclusion}")
print(f"A4B_REASON={reason}")
PY

docker image inspect "$CONTROL_IMAGE" >"$OUT/control-image.json"
docker image inspect "$BOUNDARY_IMAGE" >"$OUT/boundary-image.json"
docker --version >"$OUT/docker-version.txt"
echo "PERF003A_A4B_EXPERIMENT: PASS"
