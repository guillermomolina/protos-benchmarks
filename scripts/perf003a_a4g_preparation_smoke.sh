...
OUT=${1:-}
[ -n "$OUT" ] || { echo "usage: scripts/perf003a_a4g_preparation_smoke.sh <output-dir>" >&2; exit 2; }
...
vals={
    ...
    'VARIANT':cfg['experimental_variant'],
}
...
IMAGE="protos-benchmarks-perf003a-a4g-smoke:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4g"

echo "PERF003-A4g2 preparation-unit smoke"
...
case "$runtime" in
  ...
  *) echo "OPTIMIZING_RUNTIME_CHECK: FAIL class=$runtime" >&2; exit 10;;
esac
