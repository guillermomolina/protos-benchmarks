...
MODE=${1:-}
OUT=${2:-}
CONFIG="$ROOT/config/perf003a-a4a.json"

case "$MODE" in
    --smoke|--run) ;;
    *)
        echo "usage: scripts/perf003a_a4a_boundary_experiment.sh <--smoke|--run> <output-dir>" >&2
        exit 2
        ;;
esac
...
CONTROL_IMAGE="protos-benchmarks-perf003a-a4a-control:${PROTOS_REVISION:0:12}"
BOUNDARY_IMAGE="protos-benchmarks-perf003a-a4a-boundary:${PROTOS_REVISION:0:12}"
CTX="$ROOT/docker/protos-perf003a-a4a"

echo "PERF003-A4a controlled closure invocation boundary experiment"
echo "mode=$MODE"
...
docker run --rm --network none --entrypoint sh "$BOUNDARY_IMAGE" -c \
    "grep -B1 -A1 -F 'private static Object invokePrepared' \
...
if cg == 0:
    ...
    conclusion,reason="SUPPORTED","boundary reduced GraphTooBig occurrences"
elif cs and bs and max(x[1] for x in bs) < max(x[1] for x in cs):
    conclusion,reason="SUPPORTED","boundary reduced failing graph size"
elif cg == bg and cs == bs:
    conclusion,reason="NOT_SUPPORTED","boundary left GraphTooBig count and graph shape unchanged"
else:
    conclusion,reason="INCONCLUSIVE","boundary changed diagnostics without a directional result"
with open(out_path,"w",encoding="utf-8") as fh:
    fh.write(f"conclusion={conclusion}\nreason={reason}\n")
print(f"A4A_HYPOTHESIS={conclusion}")
print(f"A4A_REASON={reason}")
PY

docker image inspect "$CONTROL_IMAGE" >"$OUT/control-image.json"
docker image inspect "$BOUNDARY_IMAGE" >"$OUT/boundary-image.json"
docker --version >"$OUT/docker-version.txt"
echo "PERF003A_A4A_EXPERIMENT: PASS"
